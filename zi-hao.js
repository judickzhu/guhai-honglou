...
(function () {
  ...
  // 問答互動·知識庫迭代：收集提問者補充/糾錯，存 localStorage
  function getFB() {
    try { return JSON.parse(localStorage.getItem('hl_feedback') || '[]'); } catch (e) { return []; }
  }
  function addFB(q, a, ref) {
    var list = getFB();
    list.push({ q: q, a: a, ref: ref || '', source: '提問者補充', date: new Date().toISOString().slice(0,10) });
    try { localStorage.setItem('hl_feedback', JSON.stringify(list)); } catch (e) {}
    // 若本機後台在跑(http://127.0.0.1:8701),直接推送過去(自動入待審)
    try {
      var xhr = new XMLHttpRequest();
      xhr.open('POST', 'http://127.0.0.1:8701/api/ingest', true);
      xhr.setRequestHeader('Content-Type', 'application/json');
      xhr.send(JSON.stringify({ entries: [{ q: q, a: a, ref: ref || '' }] }));
    } catch (e) {}
  }
  function collectFB(question, ref) {
    var val = prompt('請補充／糾錯子嗥對「' + question + '」的回覆（寫入知識庫）：');
    if (val === null) return;
    if (!val.trim()) { alert('未寫入：內容為空。'); return; }
    addFB(question, val.trim(), ref);
    bubble(disp('收到！你的補充已記錄（localStorage），狀態<strong>待審核</strong>——不會自動入庫。管理員審核後改 status=已審，重跑生成，子嗥才學到。'), 'hint');
    updateFBCount();
  }
  function addNote() {
    var q = prompt('補充的題目（例如「中心思想」或回號）：');
    if (q === null) return;
    var val = prompt('請寫入補充／中心思想內容（狀態=待審核）：');
    if (val === null) return;
    if (!val.trim()) { alert('未寫入：內容為空。'); return; }
    addFB(q.trim() || '（未註明題目）', val.trim(), '');
    bubble(disp('收到！「'+ (q.trim()||'（未註明題目）') +'」的補充已記錄（localStorage），狀態<strong>待審核</strong>——不會自動入庫。管理員審核後重跑生成，子嗥才學到。'), 'hint');
    updateFBCount();
  }
  function updateFBCount() {
    var n = getFB().length;
    var el = document.getElementById('zh-fb-n');
    if (!el) return;
    el.textContent = n ? '(' + n + ')' : '';
  }
  function exportFB() {
    var list = getFB();
    if (!list.length) { alert('暫無補充記錄——請用「✎」或回答下「✎ 補充／糾錯」提交。'); return; }
    var json = JSON.stringify({ entries: list }, null, 2);
    try { navigator.clipboard.writeText(json); alert('已複製 ' + list.length + ' 條補充到剪貼簿，貼到 honglou_qa_feedback.json 即可。'); }
    catch (e) { prompt('複製以下 JSON 到 honglou_qa_feedback.json：', json); }
  }
  function fbBtn(question, ref) {
    var b = document.createElement('button');
    b.className = 'zh-fb';
    b.textContent = '✎ 補充／糾錯';
    b.onclick = function () { collectFB(question, ref); };
  ...
  function build() {
    ...
    root.innerHTML =
      ...
      '    <span class="zh-hd-tools"><button title="補充中心思想" onclick="ZiHao.addNote()">✎</button><button title="導出補充" onclick="ZiHao.exportFB()">⤴<span id="zh-fb-n"></span></button><button id="zh-lang" onclick="ZiHao.toggleLang()">簡</button>' +
  ...
  window.ZiHao = { open: open, close: close, toggle: toggle, ask: ask, toggleLang: toggleLang, exportFB: exportFB, addNote: addNote };
})();
